window.onload=()=>{
  ...
  gsap.set('#scrollDist', {
    ...
    onComplete:()=>{
      ...
      .fromTo('#txt1', {scale:0.5, transformOrigin:'50%'}, {scale:3, ease:'power1.in'}, 0)
      .to('#txt1 path', {duration:0.3,  stagger:0.05, ease:'power1.in'}, 0)
      .fromTo('.imgBox', {z:-5000}, {z:350, stagger:-0.3, ease:'none'}, 0)
      .fromTo('.svgBox', {z:-5000}, {z:350, stagger:-0.3, ease:'none'}, 0)
      .fromTo('.imgBox img', {scale:3}, {scale:1.15, stagger:-0.3, ease:'none'}, 0.3)
      .fromTo('.svgBox svg', {scale:3}, {scale:1.15, stagger:-0.3, ease:'none'}, 0.3)
      ...
      .to('.svgBox svg', {duration: 0.1, opacity: 0, stagger: -0.3, ease: 'expo.inOut'}, 1.2)
        .to('.imgBox, .svgBox', {duration:0, pointerEvents:'none', stagger:-0.3}, 1.27)
      .add('end')
      .fromTo('#txt2', {scale:0.05, transformOrigin:'50%'},{scale:0.6, ease:'power3'}, 'end-=0.2')
      .from('#txt2 path', {duration:0.4, ease:'sine.inOut', stagger:0.15}, 'end-=0.2')
  ...
  function initSvg(i,t){
    ...
    gsap.set(box, {
      pointerEvents: 'none',
      position: 'absolute',
      attr: { id: 'svgBox' + i, class: 'svgBox' },
      width: 500, // Match the SVG width
      height: 480, // Match the SVG height
      top: '50%',
      left: '50%',
      x: t.dataset.x,
      y: t.dataset.y,
  ...
  if (ScrollTrigger.isTouch==1) { // on mobile, hide mouse follower + remove the x/y positioning from the images
    ...
    window.onmousemove =(e)=> {
      gsap.to('.imgBox, .svgBox', { // move + rotate imgBoxes relative to mouse position
      ...
      gsap.to('.imgBox img,  .svgBox svg text', { // move images inside each imgBox, creates additional parallax effect
